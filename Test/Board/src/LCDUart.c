...
volatile U8 HarmoListPower;////默认值1，谐波列表显示中显示视在(1)、有功(2)或无功（3）电能
volatile U8 DataListPg; // 数值列表页面切换
...
** 函数说明：触摸屏任务
*/
void LCDTouchSel_Task(uint32 param)
{
#define EVENTTEST 1
...
void LCDUartView(U8 Touch_key)   // wk --> 数据显示 按键跳转函数。
{
    if(Touch_key<10|| Touch_key==255)  //  wk --> 内部页面切换或者是返回首页
    {
        ...
        /* wk@20131020--> 为了在页面切换时，谐波列表显示复位 --> merge*/
        HarmoListPhase=1;
        HarmoListUorI=1;
        HarmoListRange=1;
    ...
    case 6:
        ...
        break;
    case 7:
        Dis_PicID=MenuViewHarmoListU1; // wk --> 谐波列表显示
        break;
//    case 8:
...
//        break;   
   ...
   /* WK --> 数据显示谐波柱状图按键  */  
    ...
    /* WK --> 数据显示谐波柱状图按键  */ 
    case 0x70:  // WK --> U/I    1--> U 2-->I  3-->W
       HarmoListUorI<3 ? (++HarmoListUorI): (HarmoListUorI=1);
       MenuSwFlg=1; // 两页之间切换
       TimeFlg=1; //页面切换后完全显示时间
       if (HarmoListUorI==1) 
         Dis_PicID=MenuViewHarmoListU1;
       else if(HarmoListUorI==2)
         Dis_PicID=MenuViewHarmoListI1;
       else if(HarmoListUorI==3)
         Dis_PicID=MenuViewHarmoListP1;
       break;
//      (HarmoListUorI==1) ? (HarmoListUorI=2):(HarmoListUorI=1); // 1--> U 2-->I
//      MenuSwFlg=1; // 两页之间切换
//      TimeFlg=1; //页面切换后完全显示时间
//      /* wk @20130120-->???? 用if判断，不能正确的切换到电流界面，原因待寻 ????*/
////      if(HarmoListUorI==2)
////         Dis_PicID=MenuViewHarmoListI1;
////      else
////        Dis_PicID=MenuViewHarmoListU1;
//      (Dis_PicID==MenuViewHarmoListU1)?(Dis_PicID=MenuViewHarmoListI1):(Dis_PicID=MenuViewHarmoListU1);
//      
//      break;
    case 0x71:  // WK --> 类型  1:幅值澹  2:含有率
     ...
     if (HarmoListUorI != 3)
     {
       (HarmoListAmporRatio==1)?(HarmoListAmporRatio=2):(HarmoListAmporRatio=1);
        ...
        if( HarmoListAmporRatio==1)
          if (HarmoListUorI==1 )
            Dis_PicID=MenuViewHarmoListU1;
          else
            Dis_PicID=MenuViewHarmoListI1;
        else
            Dis_PicID=MenuViewHarmoListHYL1; 
       
     }
     else
     {
       HarmoListPower<3 ? (++HarmoListPower): (HarmoListPower=1);    
       if(HarmoListPower==1)
          Dis_PicID=MenuViewHarmoListP1;
       else if(HarmoListPower==2)
          Dis_PicID=MenuViewHarmoListQ1;
       else if(HarmoListPower==3)
          Dis_PicID=MenuViewHarmoListS1;  
     }
    break;   
        ...
        if (HarmoListAmporRatio==1)
           if(HarmoListUorI==1)
             if(HarmoListRange==1)
              ...
              Dis_PicID=MenuViewHarmoListU2;
           else
             if(HarmoListRange==1)
              Dis_PicID=MenuViewHarmoListI1;
             else
              Dis_PicID=MenuViewHarmoListI2;
        else
            if(HarmoListRange==1)
              Dis_PicID=MenuViewHarmoListHYL1;
            else
              Dis_PicID=MenuViewHarmoListHYL2;
      }
      else
      {
        if(HarmoListPower==1)  // 有功
          if(HarmoListRange==1)
            Dis_PicID=MenuViewHarmoListP1;
          else
            Dis_PicID=MenuViewHarmoListP2;
       else if(HarmoListPower==2) // 无功
          if(HarmoListRange==1)
            Dis_PicID=MenuViewHarmoListQ1;
          else
            Dis_PicID=MenuViewHarmoListQ2;
       else if(HarmoListPower==3) // 视在
          if(HarmoListRange==1)
            Dis_PicID=MenuViewHarmoListS1;  
          else
            Dis_PicID=MenuViewHarmoListS2;
      }
    ...
    case 0xFC:
     if(DataListPg==2)
     {
        ...
        MenuSwFlg=1; // 两页之间切换
        TimeFlg=1; //页面切换后完全显示时间
        if(Dis_PicID==MenuViewListMeasure2)
          Dis_PicID=MenuViewListMeasure;
        else
          Dis_PicID=MenuViewListQuality1;
     }
     break;
    ...
    case 0xFD:
      if(DataListPg==1)
      {
        ...
        MenuSwFlg=1; // 两页之间切换
        TimeFlg=1; //页面切换后完全显示时间
        if(Dis_PicID==MenuViewListMeasure)
          Dis_PicID=MenuViewListMeasure2;
        else
          Dis_PicID=MenuViewListQuality2;
      }
     break;
        
    case 254:       // wk --> hold 按键
        YADA_40(0x0000,0xf800);/* wk @131021--> 保持标志 -->merge */ 
        if (VIEWHoldFlg==0)
        {
            VIEWHoldFlg=1;
            ViewKeyFlg=0;
//            YADA_98(510,453,0x22,0x81,0x02,0xf800,0x0000,"√",0);
            ...
            /* wk @131021--> 保持标志 -->merge */           
            U16 test[]={605,457,615,467};
            YADA_40(0x0000,0xffe0);/*黄色前景，黑色背景*/
            YADA_5A(test,4);
        }
        else
        {
            VIEWHoldFlg=0;
            YADA_71(Dis_PicID,150,460,165,475,605,457);
        }
        break;
    case 255:
        Dis_PicID=MenuTop;
